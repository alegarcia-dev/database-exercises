...
SELECT first_name, last_name FROM employees GROUP BY first_name, last_name HAVING last_name LIKE 'E%E';
...
SELECT last_name, COUNT(*) FROM employees GROUP BY last_name HAVING last_name LIKE '%q%' AND last_name NOT LIKE '%qu%';

# 7
# Find all employees with first name 'Irena', 'Vidya', or 'Maya' and provide count of employees with those
# names for each gender.
SELECT first_name, gender, COUNT(*) FROM employees GROUP BY first_name, gender HAVING first_name IN ('Irena', 'Vidya', 'Maya');
...
SELECT 
	...
	COUNT(*) AS counts
FROM employees GROUP BY username ORDER BY counts DESC;
	...
	(SELECT 
		CONCAT(SUBSTR(LOWER(first_name), 1, 1),
			  SUBSTR(LOWER(last_name), 1, 4),
		 	  ...
		 	  SUBSTR(birth_date, 3, 2)) AS username,
		COUNT(*) AS counts
	FROM employees GROUP BY username HAVING COUNT(*) > 1) AS all_usernames;
...
SELECT emp_no, MAX(salary) as maximum FROM salaries GROUP BY emp_no HAVING maximum > 150000;
